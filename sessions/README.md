...
## 使い方（どの経路でも同じ）
```sh
sessions/session.sh show              # 状態・現在セッション・今日の流れを表示（開始時にまず実行）
sessions/session.sh start "見出し"     # 新しいセッションを開始
sessions/session.sh list              # セッション一覧（進行中/完了）を番号付きで表示
sessions/session.sh resume 競馬        # 既存セッションに合流（番号/ID/見出しの一部で選択）
