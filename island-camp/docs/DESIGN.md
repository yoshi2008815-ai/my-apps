...
1. **全国マップ**（SVG自前描画・観光イラストマップ調）
   - ターコイズの海×緑の陸地（白フチ海岸線）＋波・海の生きものの装飾。
     本土4島の海岸線は geo.js の実データ（`landRings()`、手描きはフォールバック）
   - デフォルメ島シェイプ（ズーム2倍以上で表示）＋ドットピン。視認性優先で実寸の1.5〜2倍
   - **沖縄インセット**: 緯度27度未満かつ経度128.7度未満の島（沖縄の島7島）は
     左上の「沖縄の島々」枠へ再投影（`project()` が `INSET` へ自動振り分け。
     本図は `projectMain()`）。与論・沖永良部など鹿児島側は本図に残る
   - **伊豆七島枠＝主役**（`IZU_BOX`/`IZU_DIPPER`/`IZU_SIZE`/`IZU_COLORS`）:
     伊豆諸島9島は右の大枠内に北斗七星ならびの固定座標で配置（`islandPoint()`）。
     各島は geo.js 実海岸線のシルエット（`izuShapePoints()`）×島別ポップカラーで
     大きめに描画。航路の寄港点も追従（FERRY_ROUTES の第4要素=島ID）
   - **鹿児島枠**（`KAGO_BOX`/`KAGO_POS`）: 種子島〜与論＋トカラ列島を左中段の
     枠内に固定座標で配置
   - **北方領土枠**（`HOPPO`）・小笠原諸島は参考地形として描画
   - 本図の描画先は `MAIN_RECT`（枠のためにcanvas全面より縮小）。
     `projectMain` が本図、`project` が沖縄インセットへの振り分けを担う
...
- 24島 × 7〜26件 = **369スポット**（2026-07 Web調査。当初9島 → 2026-07-20 に全24島へ拡充。
  2026-08-07 に八丈島の交通・電動レンタサイクル6件を追補）
...
5. **アプリの各バージョンは1コミット=1バージョン**で積んでおり、gitタグ
   `island-camp-vX.Y.Z` からいつでも過去の状態に戻せる
   （例: `git checkout island-camp-v2.4.0 -- island-camp/` で該当版のファイルを復元）。
   置き換えた旧実装（手描き海岸線 LANDMASSES・手描き道路 ROADS 等）は
   フォールバックとしてコード内に残置し、削除しない

## 13. 文書履歴

| 版 | 日付 | 対応アプリ | 主な変更 |
|---|---|---|---|
| 1.0 | 2026-07-17 | v2.1.0 | 初版（アーキテクチャ・データモデル・同期設計） |
| 1.1 | 2026-07-17 | v2.2.0 | 横並びビュー追記 |
| 1.2 | 2026-07-20 | v2.5.0〜v2.6.0 | 観光マップ実データ化・公式マップPDFビューア・ファイル構成更新 |
| 1.3 | 2026-07-20 | v2.7.0〜v2.10.0 | 全24島シード・航路レイヤー・全国マップ刷新（インセット枠・北斗七星ならび・MAIN_RECT）・本節と復元手順を追記 |
| 1.4 | 2026-08-07 | v2.11.0 | 八丈島を2026年の島に追加（シード島への updatedAt 付与を初適用）・交通/電動レンタサイクル6スポット追補（369スポット） |
